...
class SqsLocalPlugin {
      ...
      sqs: {
        commands: {
          ...
          remove: {
              ...
              usage: "Removes all queues listed in the current Serverless configuration."
          }
        }
      }
    };
  ...
  removeHandler() {
      ...
      return this.removeQueues(queuesInConfiguration).then(removed => {
          removed.filter(n => n).forEach(removedQueue => this.log(`Deleted ${removedQueue}`));
  ...
  removeQueue(queue) {
      return this.getQueueUrl(queue.QueueName).then(url => {
          return new Promise((resolve, reject) => {
